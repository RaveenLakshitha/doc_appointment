...
class WhatsAppFlowController extends Controller
{
    ...
    private function decryptPayload(array $body): ?array
    {
        try {
            ...
            if (!$aesKey) throw new \Exception('AES key decryption failed');
            ...
            if (!$decrypted) throw new \Exception('Flow data decryption failed');
    ...
    private function processFlow(string $action, string $screen, array $data, ?string $flowToken): array
    {
        ...
        if ($screen === 'SPECIALIZATION' && $action === 'data_exchange') {
            if (($data['trigger'] ?? '') === 'specialization_selected') {
                $specId = $data['specialization'] ?? null;
            ...
            $response['data']['specialization'] = $data['specialization'] ?? null;
            return $response;
        }
        ...
        if ($screen === 'DOCTOR_SELECT' && $action === 'data_exchange') {
            ...
            if (!$doctorId || !Doctor::where('id', $doctorId)->active()->exists()) {
        ...
        if ($screen === 'VISIT_TYPE' && $action === 'data_exchange') {
            ...
            $response['data']['doctor'] = $data['doctor'] ?? null;
            $response['data']['specialization'] = $data['specialization'] ?? null;
            return $response;
        }
        ...
        if ($screen === 'REASON' && $action === 'data_exchange') {
            ...
            $response['data']['summary_text'] = 'Review your appointment details below';
        ...
        if ($screen === 'SUMMARY' && $action === 'data_exchange' && !empty($data['complete'])) {
            ...
            if ($data['appointment_type'] === 'specific') {
                $validator->sometimes('doctor', 'required|exists:doctors,id', fn() => true);
            } else {
                $validator->sometimes('specialization', 'required|exists:specializations,id', fn() => true);
            }
