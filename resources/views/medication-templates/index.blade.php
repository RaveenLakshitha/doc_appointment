...
@section('title', __('file.medication_templates'))
...
<div class="px-4 sm:px-6 lg:px-4 py-4 sm:py-6">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h1 class="text-2xl sm:text-3xl font-semibold text-gray-900 dark:text-white">
                {{ __('file.medication_templates') }}
            </h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{ __('file.manage_medication_templates') }}
            </p>
        </div>
        ...
        <div class="flex flex-row-reverse sm:flex-row gap-3 w-full sm:w-auto justify-between sm:justify-end">
            ...
            <a href="{{ route('medication-templates.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition shadow-sm">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                </svg>
                <span class="hidden sm:inline">{{ __('file.add_template') }}</span>
    ...
    <!-- Filter Drawer (Mobile: bottom sheet | Desktop: right sidebar) -->
    ...
    <div id="bulk-delete-form" class="hidden mb-6">
        <form method="POST" action="{{ route('medication-templates.bulkDelete') }}" class="bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-800 rounded-lg p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            @csrf @method('DELETE')
            <input type="hidden" name="ids" id="bulk-ids">
            <span class="text-sm font-medium text-red-800 dark:text-red-300">
                <span id="selected-count">0</span> {{ __('file.template_selected') }}
            </span>
            ...
            <table id="docapp-table" class="w-full divide-y divide-gray-200 dark:divide-gray-700 display nowrap" style="width:100%">
                <thead class="bg-gray-50 dark:bg-gray-900">
                    <tr>
                        ...
                        <th class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider desktop">{{ __('file.medications_count') }}</th>
                        <th class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider desktop">{{ __('file.last_used') }}</th>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Drawer open/close logic (same as doctors)
        function openDrawer() {
        ...
        $.get('{{ route("medication-templates.filters") }}', { column: 'category' }, data => {
        ...
        const table = $('#docapp-table').DataTable({
            ...
            responsive: true,
            ajax: {
                url: '{{ route("medication-templates.datatable") }}',
                data: d => {
            ...
            columns: [
                ...
                { data: 'description', render: data => `<span class="text-gray-600 dark:text-gray-400 text-sm">${data ? data.substring(0, 80) + (data.length > 80 ? '...' : '') : '-'}</span>` },
                {
                    data: 'category',
                    render: (data, type, row) => data ?
                        `<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium" style="background-color: ${row.category_color}20; color: ${row.category_color}">
                            ${data}
                        </span>` : '-'
                },
                { data: 'medications_count', render: data => data ?? 0 },
                { data: 'last_used_at', render: data => data ? new Date(data).toLocaleDateString() + ' (' + moment(data).fromNow() + ')' : 'Never' },
                {
                    data: null,
                    orderable: false,
                    searchable: false,
                    ...
                    render: (data, type, row) => `
                        <div class="flex items-center justify-end gap-1">
                            ...
                            <form method="POST" action="${row.delete_url}" class="inline">
                                @csrf @method('DELETE')
                                <button type="submit" onclick="return confirm('{{ __('file.confirm_delete_template') }}')" class="p-1.5 sm:p-2 text-gray-600 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-500 transition-colors" title="{{ __('file.delete') }}">
            ...
            layout: {
                topStart: {
                    buttons: [
                        ...
                        {
                            extend: 'collection',
                            text: '<span class="hidden sm:inline">{{ __('file.Export') }}</span><span class="sm:hidden">Download</span>',
            ...
            language: {
                ...
                infoEmpty: "{{ __('file.no_templates_found') }}",
        ...
        document.getElementById('clear-filters').addEventListener('click', () => { filterCategory.value = ''; table.draw(); updateFilterCount(); });
        ...
        $('#bulk-delete-form form').on('submit', function (e) {
            e.preventDefault();
            if (confirm('{{ __("file.confirm_delete_selected_templates") }}')) {
                $.ajax({
                    url: this.action,
                    method: 'POST',
                    data: $(this).serialize(),
                    success: () => { table.draw(false); updateBulkDelete(); },
                    error: () => alert('Error deleting templates.')
                });
            }
        });
