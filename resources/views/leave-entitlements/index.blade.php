...
@section('title', __('file.leave_entitlements'))
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{ __('file.manage_employee_leave_balances') }}
            </p>
        </div>
        ...
        <div class="flex flex-row-reverse sm:flex-row gap-3 w-full sm:w-auto justify-between sm:justify-end">
            ...
            <a href="{{ route('leave-entitlements.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition shadow-sm">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                </svg>
                <span class="hidden sm:inline">{{ __('file.assign_leave_entitlement') }}</span>
        ...
        <div class="p-6 overflow-y-auto max-h-[calc(85vh-140px)] md:max-h-[calc(100vh-140px)]">
            <div class="space-y-5">
                <div>
                    ...
                    <select id="filter-employee" class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
                        <option value="">{{ __('file.all_employees') }}</option>
                    </select>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('file.year') }}</label>
                    <select id="filter-year" class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
                        <option value="">{{ __('file.all_years') }}</option>
            ...
            <table id="docapp-table" class="w-full divide-y divide-gray-200 dark:divide-gray-700 display nowrap" style="width:100%">
                <thead class="bg-gray-50 dark:bg-gray-900">
                    <tr>
                        ...
                        <th class="px-4 sm:px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider desktop">{{ __('file.year') }}</th>
                        <th class="px-4 sm:px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider desktop">{{ __('file.entitled_days') }}</th>
                        <th class="px-4 sm:px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider desktop">{{ __('file.used_days') }}</th>
                        <th class="px-4 sm:px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider desktop">{{ __('file.remaining_days') }}</th>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateFilterCount() {
        const count = [filterEmployee.value, filterYear.value].filter(Boolean).length;
        if (count > 0) {
    ...
    // Populate employee dropdown (assuming you have a route like attendances.filters)
    ...
    const table = $('#docapp-table').DataTable({
        ...
        responsive: true,
        ajax: {
            url: '{{ route("leave-entitlements.datatable") }}',
            data: function(d) {
                d.employee_id = filterEmployee.value;
                d.year = filterYear.value;
            }
        },
        ...
        columns: [
            ...
            { data: 'entitled', className: 'text-center' },
            { data: 'used', className: 'text-center' },
            { data: 'remaining_html', className: 'text-center' },
            {
                data: null,
                orderable: false,
                searchable: false,
        ...
        language: {
            search: "",
            searchPlaceholder: "{{ __('file.search_entitlements') }}",
            emptyTable: "{{ __('file.no_entitlements_found') }}"
        }
    });
    ...
    [filterEmployee, filterYear].forEach(el => el.addEventListener('change', updateFilterCount));
