...
@section('title', __('file.Create Payroll') ?? 'Create Conciliation')
...
<div class="px-4 sm:px-6 lg:px-4 pb-4 sm:py-12 pt-20" x-data="payrollForm()">
    <div class="space-y-6">
        ...
        <div class=" mb-6">
            <div class="flex items-center text-sm text-gray-500 dark:text-gray-400 mb-3">
                ...
                <span class="text-gray-900 dark:text-white">{{ __('file.Create Payroll') ?? 'Create Conciliation' }}</span>
            </div>
            <h1 class="text-3xl font-semibold text-gray-900 dark:text-white">{{ __('file.Create Payroll') ?? 'Create Conciliation' }}</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('file.create_payroll_desc') }}</p>
        </div>
                ...
                <div class="p-6 space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="recipient_type" x-model="recipientType" @change="loadAppointments" required
                                ...
                                <option value="" disabled selected>{{ __('file.select_type') }}</option>
                        ...
                        <div>
                            ...
                            <div x-show="recipientType === 'doctor'" x-cloak>
                                <select x-bind:name="recipientType === 'doctor' ? 'recipient_id' : ''" x-model="recipientId" @change="loadAppointments" :required="recipientType === 'doctor'"
                                    class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-600 dark:focus:ring-indigo-400 focus:border-transparent dark:bg-gray-900 dark:text-white transition-shadow">
                                    <option value="" disabled selected>{{ __('file.select_therapist') }}</option>
                            ...
                            <div x-show="recipientType === 'employee'" x-cloak>
                                <select x-bind:name="recipientType === 'employee' ? 'recipient_id' : ''" x-model="recipientId" :required="recipientType === 'employee'"
                                    class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-600 dark:focus:ring-indigo-400 focus:border-transparent dark:bg-gray-900 dark:text-white transition-shadow">
                                    <option value="" disabled selected>{{ __('file.select_employee') }}</option>
                            ...
                            <div x-show="!recipientType" class="w-full px-3 py-2 text-sm border border-dashed border-gray-300 dark:border-gray-600 rounded-lg text-gray-400 dark:text-gray-500">
                                {{ __('file.select_recipient_type_first') }}
                            </div>
                            ...
                            <template x-for="app in appointments" :key="app.id">
                                <label class="flex items-start p-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl cursor-pointer hover:border-indigo-500 dark:hover:border-indigo-400 transition-all shadow-sm">
                                    <div class="flex-shrink-0 mt-1">
                                        <input type="checkbox" name="appointments[]" :value="app.id" class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 appointment-checkbox">
                                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-5 gap-6 pt-6 border-t border-gray-100 dark:border-gray-700">
                        ...
                        <div>
                            ...
                            <select name="payment_method" required
                                ...
                                <option value="Bank Transfer">{{ __('file.bank_transfer') }}</option>
                                <option value="Check">{{ __('file.check') }}</option>
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="4"
                            ...
                            placeholder="{{ __('file.internal_notes_placeholder') }}"></textarea>
                    </div>
                </div>
    ...
    <div id="invoice-modal" class="fixed inset-0 z-[60] hidden">
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4"
            ...
            <!-- Modal Card -->
            <div class="relative w-full max-w-4xl max-h-[90vh] flex flex-col bg-white dark:bg-gray-800 rounded-xl shadow-xl overflow-hidden">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white" id="modal-title">
                        {{ __('file.invoice_details') ?? 'Bill Details' }}
                    </h3>
                    <button type="button" onclick="closeInvoiceModal()"
                        class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 focus:outline-none p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                ...
                <div class="relative flex-1 w-full bg-gray-50 dark:bg-gray-900 p-6 min-h-[300px] overflow-y-auto">
                    <div id="modal-loader"
                    ...
                    <div id="invoice-summary-content" class="text-gray-900 dark:text-gray-100"></div>
                </div>
            </div>
        </div>
    </div>
...
<script>
    function payrollForm() {
        return {
            ...
            loadAppointments() {
                ...
                let url = `{{ route('payrolls.get-appointments') }}?doctor_id=${this.recipientId}&month=${this.selectedMonth}`;
            ...
            openInvoice(id) {
                if (!id) return;
                const viewUrl = `{{ route('payrolls.invoice-summary', ':id') }}`.replace(':id', id);
                const invoiceModal = document.getElementById('invoice-modal');
                const summaryContent = document.getElementById('invoice-summary-content');
                ...
                fetch(viewUrl)
                    ...
                    .catch(err => {
                        console.error(err);
                        summaryContent.innerHTML = '<p class="text-red-500">Failed to load details.</p>';
                        modalLoader.style.display = 'none';
                    });
            }
        }
    ...
    // Global close function (used by onclick & ESC key)
