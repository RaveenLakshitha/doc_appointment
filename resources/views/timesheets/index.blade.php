...
@section('title', __('file.timesheets'))
...
<div class="px-4 sm:px-6 lg:px-4 py-4 sm:py-6">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{ __('file.manage_timesheet_records') }}
            </p>
        </div>
        ...
        <div class="flex flex-row-reverse sm:flex-row gap-3 w-full sm:w-auto justify-between sm:justify-end">
            ...
            <a href="{{ route('timesheets.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition shadow-sm">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                </svg>
                <span class="hidden sm:inline">{{ __('file.add_timesheet') }}</span>
    ...
    <div class="relative bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div class="overflow-x-auto">
            <table id="timesheet-table" class="w-full divide-y divide-gray-200 dark:divide-gray-700 display nowrap" style="width:100%">
                <thead class="bg-gray-50 dark:bg-gray-900">
                    <tr>
                        ...
                        <th class="px-4 sm:px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider desktop">{{ __('file.start_time') }}</th>
                        <th class="px-4 sm:px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider desktop">{{ __('file.end_time') }}</th>
                        <th class="px-4 sm:px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider desktop">{{ __('file.hours_worked') }}</th>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateFilterCount() {
        const count = [filterEmployee.value, filterDepartment.value, filterDateFrom.value, filterDateTo.value].filter(Boolean).length;
        if (count > 0) {
    ...
    $.get('{{ route("timesheets.filters") }}', { column: 'employee' }, data => {
        $.each(data, (id, name) => $('#filter-employee').append(`<option value="${id}">${name}</option>`));
    });

    $.get('{{ route("timesheets.filters") }}', { column: 'department' }, data => {
    ...
    const table = $('#timesheet-table').DataTable({
        ...
        responsive: true,
        ajax: {
            url: '{{ route("timesheets.datatable") }}',
            data: function(d) {
        ...
        columns: [
            ...
            { data: 'hours_worked', className: 'text-center font-medium' },
        ...
        language: {
            search: "",
            searchPlaceholder: "{{ __('file.search_timesheets') }}",
            emptyTable: "{{ __('file.no_timesheet_records') }}"
        }
    });

    document.getElementById('apply-filters').addEventListener('click', () => { table.draw(); updateFilterCount(); closeDrawerFunc(); });
    document.getElementById('clear-filters').addEventListener('click', () => {
        filterEmployee.value = ''; filterDepartment.value = ''; filterDateFrom.value = ''; filterDateTo.value = '';
        table.draw(); updateFilterCount();
    });

    [filterEmployee, filterDepartment, filterDateFrom, filterDateTo].forEach(el => el.addEventListener('change', updateFilterCount));
