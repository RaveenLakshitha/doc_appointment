            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 sticky top-6">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <div class="text-xl font-bold text-blue-600 dark:text-blue-400">{{ $doctor->appointments_count }}</div>
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700" x-data="{ activeTab: 'profile' }">
                ...
                <div class="p-6">
                    <div x-show="activeTab === 'profile'" x-cloak>
                        <div class="space-y-8">
                            <div>
                                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('file.personal_information') }}</h3>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('file.first_name') }}</label>
                                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $doctor->first_name ?? '—' }}</p>
                                    </div>
                                    <div>
                                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('file.middle_name') }}</label>
                                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $doctor->middle_name ?? '—' }}</p>
                                    </div>
                                    <div>
                                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('file.last_name') }}</label>
                                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $doctor->last_name ?? '—' }}</p>
                                    </div>
                                    <div>
                                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('file.date_of_birth') }}</label>
                                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $doctor->date_of_birth?->format('d M Y') ?? '—' }}</p>
                                    </div>
                                    <div>
                                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('file.gender') }}</label>
                                        <p class="mt-1 text-sm text-gray-900 dark:text-white capitalize">{{ $doctor->gender ?? '—' }}</p>
                                    </div>
                                    <div>
                                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('file.phone') }}</label>
                                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $doctor->phone ?? '—' }}</p>
                                    </div>
                                    <div>
                                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('file.email') }}</label>
                                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $doctor->email ?? '—' }}</p>
                                    </div>
                                </div>
                                ...
                                @if($doctor->emergency_contact_name || $doctor->emergency_contact_phone)
                                    <div class="mt-6">
                                        <h4 class="text-sm font-semibold text-gray-900 dark:text-white mb-2">{{ __('file.emergency_contact') }}</h4>
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('file.license_expiry_date') }}</label>
                                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $doctor->license_expiry_date?->format('d M Y') ?? '—' }}</p>
                                    </div>
                                    <div>
                                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('file.years_experience') }}</label>
                                ...
                                <div class="mt-6 space-y-6">
                                    @if($doctor->qualifications)
                                        <div>
                                            <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('file.qualifications') }}</label>
                                            <p class="mt-1 text-sm text-gray-900 dark:text-white whitespace-pre-line">{{ $doctor->qualifications }}</p>
                                        </div>
                                    @endif

                                    @if($doctor->education)
                                        <div>
                                            <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('file.education') }}</label>
                                            <p class="mt-1 text-sm text-gray-900 dark:text-white whitespace-pre-line">{{ $doctor->education }}</p>
                                        </div>
                                    @endif

                                    @if($doctor->certifications)
                                        <div>
                                            <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('file.certifications') }}</label>
                                            <p class="mt-1 text-sm text-gray-900 dark:text-white whitespace-pre-line">{{ $doctor->certifications }}</p>
                                        </div>
                                    @endif

                                    @if(!$doctor->qualifications && !$doctor->education && !$doctor->certifications)
                                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('file.no_additional_info') }}</p>
                                    @endif
                                </div>
                                ...
                                <div class="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div>
                                        <h4 class="text-sm font-semibold text-gray-900 dark:text-white mb-3">{{ __('file.age_groups_treated') }}</h4>
                                        @if($doctor->ageGroups->isNotEmpty())
                                            <div class="flex flex-wrap gap-2">
                                                @foreach($doctor->ageGroups as $group)
                                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-indigo-100 dark:bg-indigo-900/40 text-indigo-800 dark:text-indigo-300">
                                                        {{ $group->name }}
                                    ...
                                    <div>
                                        <h4 class="text-sm font-semibold text-gray-900 dark:text-white mb-3">{{ __('file.languages_spoken') }}</h4>
                                        @if($doctor->languages->isNotEmpty())
                                            <div class="flex flex-wrap gap-2">
                                                @foreach($doctor->languages as $lang)
                                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 dark:bg-green-900/40 text-green-800 dark:text-green-300">
                                                        {{ $lang->name }}
                                                    </span>
                                                @endforeach
                                            </div>
                                ...
                                <!-- Treatments & Prices -->
                                <div class="mt-8">
                                    <h4 class="text-sm font-semibold text-gray-900 dark:text-white mb-3">{{ __('file.treatments_offered') }} & Prices</h4>
                                    @if($doctor->treatments->isNotEmpty())
                                        <div class="overflow-x-auto">
                                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                                <thead class="bg-gray-50 dark:bg-gray-800">
                                                    <tr>
                                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Treatment</th>
                                                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Price {{ $currency_code }}</th>
                                                    </tr>
                                                </thead>
                                                    ...
                                                    @foreach($doctor->treatments as $treatment)
                                                        <tr>
                                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                                                                {{ $treatment->name }} {{ $treatment->code ? "({$treatment->code})" : '' }}
                                                            </td>
