...
<div class="px-4 sm:px-6 lg:px-4 pb-4 sm:py-12 pt-20">
    ...
    <div id="bulk-delete-form" class="hidden mb-6">
        <form method="POST" action="{{ route('age-groups.bulkDelete') }}" class="bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-800 rounded-lg p-4 flex justify-between items-center">
            @csrf @method('DELETE')
        ...
        <table id="docapp-table" class="w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-900">
                <tr>
                    ...
                    <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('file.age_range') }}</th>
...
<div id="profile-drawer" class="fixed inset-0 z-50 hidden overflow-hidden">
    ...
    <div id="profile-panel" class="absolute inset-x-0 bottom-0 md:inset-y-0 md:right-0 md:left-auto w-full md:max-w-md bg-white dark:bg-gray-800 shadow-2xl flex flex-col h-[90vh] md:h-full rounded-t-3xl md:rounded-none">
        ...
        <div class="flex-1 overflow-y-auto px-5 py-5 text-sm space-y-6">
            <div>
                <h4 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">{{ __('file.age_range') }}</h4>
        ...
        <form id="edit-form" class="flex-1 flex flex-col">
            ...
            <div class="flex-1 overflow-y-auto px-5 py-5 text-sm space-y-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <h4 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">{{ __('file.min_age') }}</h4>
                        <input type="number" name="min_age" id="edit-min_age" min="0" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 text-gray-900 dark:text-white" />
                    </div>
                    <div>
                        <h4 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">{{ __('file.max_age') }}</h4>
                        <input type="number" name="max_age" id="edit-max_age" min="0" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 text-gray-900 dark:text-white" />
                    </div>
                </div>
                <div>
                    <h4 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">{{ __('file.description') }}</h4>
                    <textarea name="description" id="edit-description" rows="5" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 text-gray-900 dark:text-white"></textarea>
                </div>
                <div>
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" id="edit-is_active" value="1" class="w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
            ...
            { data: null, orderable: false, searchable: false, className: 'text-right whitespace-nowrap', render: (data, type, row) => `
                <div class="flex items-center justify-end gap-1">
                    ...
                    <form method="POST" action="${row.delete_url}" class="inline">
                        @csrf @method('DELETE')
                        <button type="submit" onclick="return confirm('{{ __('file.confirm_delete_age_group') }}')"
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        buttons: [
                            ...
                            { extend: 'excel', text: 'Excel', filename: 'AgeGroups_{{ date("Y-m-d") }}' },
                            { extend: 'csv', text: 'CSV', filename: 'AgeGroups_{{ date("Y-m-d") }}' },
                            { extend: 'pdf', text: 'PDF', filename: 'AgeGroups_{{ date("Y-m-d") }}', title: 'Age Group List' },
    ...
    document.getElementById('edit-form').addEventListener('submit', function(e) {
        ...
        .then(data => {
            if (data.success) {
                table.draw(false);
                closeEditDrawer();
                alert(data.message || '{{ __('file.age_group_updated_successfully') }}');
            }
        })
        .catch(error => {
