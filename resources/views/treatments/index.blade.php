...
<div class="px-4 sm:px-6 lg:px-4 pb-4 sm:py-12 pt-20">
    ...
    <div id="bulk-delete-form" class="hidden mb-6">
        <form method="POST" action="{{ route('treatments.bulkDelete') }}" class="bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-800 rounded-lg p-4 flex justify-between items-center">
            @csrf
            ...
            <table id="docapp-table" class="w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-900">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('file.code') }}</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('file.appointments') }}</th>
...
<div id="create-drawer" class="fixed inset-0 z-50 hidden overflow-hidden">
    ...
    <div id="create-panel" class="absolute inset-x-0 bottom-0 md:inset-y-0 md:right-0 md:left-auto w-full md:max-w-md bg-white dark:bg-gray-800 shadow-2xl flex flex-col h-[90vh] md:h-full rounded-t-3xl md:rounded-none">
        ...
        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-700">
            <div>
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('file.create_new_treatment') }}</h3>
        ...
        <div class="flex-1 overflow-y-auto px-5 py-5 text-sm">
            <form id="create-form" class="space-y-5">
                @csrf
                <div>
                    <label class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('file.name') }} <span class="text-red-500">*</span></label>
                    <input type="text" name="name" id="create-name" required class="mt-2 w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 text-gray-900 dark:text-white" placeholder="e.g. Root Canal, Dental Cleaning" />
                </div>
                <div class="flex items-center">
                    <input type="checkbox" name="active" id="create-active" value="1" checked class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded dark:bg-gray-700 dark:border-gray-600">
                    <label for="create-active" class="ml-2 block text-sm text-gray-900 dark:text-gray-300">{{ __('file.active_visible_for_doctors') }}</label>
                </div>
            </form>
        </div>
...
<div id="profile-drawer" class="fixed inset-0 z-50 hidden overflow-hidden">
    ...
    <div id="drawer-panel" class="absolute inset-x-0 bottom-0 md:inset-y-0 md:right-0 md:left-auto w-full md:max-w-md bg-white dark:bg-gray-800 shadow-2xl flex flex-col h-[90vh] md:h-full rounded-t-3xl md:rounded-none">
        ...
        <div class="flex-1 overflow-y-auto px-5 py-5 text-sm">
            <div class="space-y-6">
                <div>
                    <h4 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">{{ __('file.basic_info') }}</h4>
                ...
                <div>
                    <h4 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">{{ __('file.usage') }}</h4>
                    <p id="drawer-usage" class="text-gray-900 dark:text-white"></p>
                </div>
            </div>
        </div>
        <div class="px-5 py-3 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700">
            <button onclick="closeProfileDrawer()" class="w-full px-4 py-2 bg-gray-900 dark:bg-gray-700 text-white text-sm font-medium rounded-lg hover:bg-gray-800 dark:hover:bg-gray-600 transition">{{ __('file.close') }}</button>
        </div>
    </div>
</div>
    ...
    <div id="edit-panel" class="absolute inset-x-0 bottom-0 md:inset-y-0 md:right-0 md:left-auto w-full md:max-w-md bg-white dark:bg-gray-800 shadow-2xl flex flex-col h-[90vh] md:h-full rounded-t-3xl md:rounded-none">
        ...
        <div class="flex-1 overflow-y-auto px-5 py-5 text-sm">
            <form id="edit-form" class="space-y-5">
                ...
                <div>
                    <label class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('file.code') }} (auto-generated)</label>
                    <input type="text" id="edit-code" class="mt-2 w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-200 dark:bg-gray-600 text-gray-900 dark:text-white cursor-not-allowed" disabled>
                ...
                <div>
                    ...
                    <select name="active" id="edit-active" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 text-gray-900 dark:text-white">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    $('#bulk-delete-form form').on('submit', function (e) {
        ...
        $.ajax({
            ...
            error: (xhr) => {
                alert(xhr.responseJSON?.message || 'Error deleting selected treatments');
            }
        });
    });
    ...
    document.getElementById('create-form')?.addEventListener('submit', function(e) {
        ...
        .then(data => {
            if (data.success) {
                table.draw(false);
                closeCreateDrawer();
                alert(data.message || 'Treatment created successfully');
            } else {
        ...
        .catch(() => alert('Failed to create treatment'));
    });
    ...
    document.getElementById('edit-form').addEventListener('submit', function(e) {
        ...
        .catch(() => alert('Failed to update treatment'));
    });
    ...
    window.deleteTreatment = function(url) {
        ...
        .then(data => {
            if (data.success) {
                ...
                alert(data.message || 'Cannot delete this treatment');
            }
        })
