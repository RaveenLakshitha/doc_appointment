...
<div class="px-4 sm:px-6 lg:px-4 pb-4 sm:py-12 pt-20">
    <div class="mb-8">
        <div class="flex items-center text-sm text-gray-500 dark:text-gray-400 mb-3">
            <a href="{{ route('medicine-templates.index') }}" class="hover:text-gray-700 dark:hover:text-gray-300 transition-colors">
        ...
        <div class="p-6 space-y-8">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                                    ...
                                    @if($medication->instructions)
                                        <div class="sm:col-span-2 mt-3">
                                            ...
                                            <p class="mt-1 text-gray-900 dark:text-gray-200 whitespace-pre-wrap">{{ $medication->instructions }}</p>
                                        </div>
                                    @endif
                                </div>
                            </div>
            ...
            <div id="content-details" class="tab-content hidden">
                <div class="space-y-6 max-w-3xl">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('file.template_name') }}</label>
                        <p class="mt-1 text-lg font-medium text-gray-900 dark:text-white">{{ $medicineTemplate->name }}</p>
                    </div>

                    @if($medicineTemplate->category)
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('file.category') }}</label>
                    ...
                    @if($medicineTemplate->description)
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('file.description') }}</label>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('file.total_medications') }}</label>
                            <p class="mt-1 text-lg text-gray-900 dark:text-white">{{ $medicineTemplate->medications->count() }}</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('file.created_at') }}</label>
                        ...
                        @if($medicineTemplate->updated_at && $medicineTemplate->updated_at->notEqualTo($medicineTemplate->created_at))
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('file.last_updated') }}</label>
                                <p class="mt-1 text-gray-600 dark:text-gray-400">
