...
<div class="px-4 sm:px-6 lg:px-4 pb-4 sm:py-12 pt-20">
    ...
    <!-- Filter Drawer (consistent with Patients, Prescriptions, etc.) -->
    <div id="filter-drawer" class="fixed inset-0 z-50 hidden">
        ...
        <div class="fixed inset-y-0 right-0 w-full max-w-md bg-white dark:bg-gray-800 shadow-xl transform translate-x-full transition-transform duration-300 ease-in-out" id="drawer-panel">
            ...
            <div class="p-6 overflow-y-auto h-full pb-32">
                <div class="space-y-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('file.role') }}</label>
                        <select id="filter-role" class="w-full px-3 py-2 text-sm rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:ring-2 focus:ring-indigo-500">
                    ...
                    <div>
                        <h4 class="text-sm font-semibold text-gray-900 dark:text-white mb-3 uppercase tracking-wider">{{ __('file.created_date') }}</h4>
            ...
            { data: null, className: 'text-right whitespace-nowrap', render: (data, type, row) => `
                <div class="flex items-center justify-end gap-1">
                    ...
                    <form method="POST" action="${row.delete_url}" class="inline">
                        @csrf @method('DELETE')
                        <button type="submit" onclick="return confirm('{{ __('file.soft_delete_confirm') }}')"
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        buttons: [
                            ...
                            { extend: 'excel', text: 'Excel', filename: 'Users_{{ date("Y-m-d") }}' },
                            { extend: 'csv', text: 'CSV', filename: 'Users_{{ date("Y-m-d") }}' },
                            { extend: 'pdf', text: 'PDF', filename: 'Users_{{ date("Y-m-d") }}', title: 'User List' },
