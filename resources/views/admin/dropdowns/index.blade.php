...
<div class="px-4 sm:px-6 lg:px-4 pb-4 sm:py-12 pt-20">
    ...
    <div id="option-backdrop" class="hidden fixed inset-0 bg-black bg-opacity-50 z-40 transition-opacity duration-300 opacity-0"></div>

    <div id="option-drawer" class="fixed z-50 bg-white dark:bg-gray-800 shadow-2xl transition-transform duration-300 ease-in-out
        ...
        <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            ...
            <button type="button" id="close-option-drawer" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5">
        ...
        <div class="p-6 overflow-y-auto max-h-[calc(85vh-140px)] md:max-h-[calc(100vh-140px)]">
            <form id="option-form" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <select name="type" id="option-type" required
                        ...
                        @foreach($groupedOptions as $type => $options)
                            <option value="{{ $type }}" 
                                    {{ old('type') ? ($old('type') == $type ? 'selected' : '') : ($loop->first ? 'selected' : '') }}>
                                {{ __('file.' . $type) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="status" id="option-status" class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
        ...
        <div class="bottom-0 left-0 right-0 flex gap-3 px-6 py-4 bg-gray-50 dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700">
            <button type="button" id="cancel-option" class="flex-1 px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors">
                {{ __('file.cancel') }}
            </button>
            ...
            <table class="w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-900">
                    <tr>
                        ...
                        <th class="px-4 sm:px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider no-export">
                            {{ __('file.actions') }}
                        </th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                            ...
                            @if($options->isNotEmpty())
                                <div class="flex flex-wrap gap-2">
                                    @foreach($options as $option)
                                        <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
                                            {{ $option->name }}
                                            @if(!$option->status)
                                                <span class="text-red-500 dark:text-red-400">(inactive)</span>
                                            @endif
                                            <button type="button" class="edit-option ml-1 text-blue-600 dark:text-blue-300 hover:text-blue-800"
                                                    data-id="{{ $option->id }}"
                        ...
                        <td class="px-4 sm:px-6 py-4 text-right whitespace-nowrap">
                            <button type="button" class="add-option p-1.5 text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"
                                    data-type="{{ $type }}">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                                </svg>
