        ...
        <div class="mb-8 border-b border-gray-200 dark:border-gray-700">
            <nav class="flex space-x-8" aria-label="Tabs">
                ...
                <button onclick="switchTab('notifications')" id="tab-notifications" class="tab-btn ... relative">
                    Notifications
                    @if($unreadCount > 0)
                        <span class="absolute top-3 right-0 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-white transform translate-x-1/2 -translate-y-1/2 bg-red-500 rounded-full">
                            {{ $unreadCount }}
                        </span>
                    @endif
                </button>
            ...
            <div class="mb-6 flex items-center justify-between">
                ...
                <div class="flex space-x-4">
                    <form action="{{ route('notifications.mark-all-read') }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" class="px-4 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">
                            Mark All as Read
                        </button>
                    </form>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow divide-y divide-gray-200 dark:divide-gray-700">
                @if($notifications->isEmpty())
                    <div class="p-12 text-center">
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">We'll let you know when something new arrives.</p>
                    </div>
                @else
                    @foreach($notifications as $notification)
                        <div class="p-5 notification-item {{ $notification->read_at ? 'opacity-60 bg-gray-50 dark:bg-gray-900/50' : 'bg-blue-50 dark:bg-blue-900/20' }} hover:bg-gray-100 dark:hover:bg-gray-800/50 transition">
