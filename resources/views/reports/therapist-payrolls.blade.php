...
@section('title', __('file.therapist_payrolls_report') ?? 'Therapist Conciliations Report')

@section('content')

    <div class="px-4 sm:px-6 lg:px-8 pb-4 sm:py-12 pt-20">
        <div class="mb-6">
            <h1 class="text-2xl sm:text-3xl font-semibold text-gray-900 dark:text-white">{{ __('file.therapist_payrolls_report') ?? 'Therapist Monthly Conciliations Report' }}</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{ __('file.track_therapist_payrolls') ?? 'View a daily breakdown of a therapist\'s conciliation payouts for a selected month.' }}
            </p>
        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                <div class="border rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white shadow-sm">
                    <div class="p-4 flex flex-row items-center justify-between space-y-0 pb-2">
                        <h3 class="tracking-tight text-sm font-medium">{{ __('file.days_paid') ?? 'Days Paid' }}</h3>
                    ...
                    <div class="p-4 pt-0">
                        <div class="text-2xl font-bold text-gray-700 dark:text-gray-300" id="total_days_paid">0</div>
                    </div>
                </div>

                <div class="border rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white shadow-sm">
                    <div class="p-4 flex flex-row items-center justify-between space-y-0 pb-2">
                        <h3 class="tracking-tight text-sm font-medium">{{ __('file.total_overall') ?? 'Total Overall' }}</h3>
                    ...
                    <div class="p-4 pt-0">
                        <div class="text-2xl font-bold text-indigo-600 dark:text-indigo-400" id="total_overall">0</div>
                    </div>
                </div>
                    ...
                    <table class="w-full min-w-max">
                        <thead class="bg-gray-50 dark:bg-gray-900 sticky top-0 z-10">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                    {{ __('file.payrolls') ?? 'Conciliations Count' }}
                                </th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                    {{ __('file.cash') ?? 'Cash' }}
                                </th>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                function generateReport() {
                    ...
                    fetch(`{{ route('reports.therapist-payrolls.data') }}?${params}`)
                ...
                function updateSummaryCards(summary) {
                    document.getElementById('total_days_paid').textContent = summary.total_days_paid;
                    
                    const formatCurrency = (val) => '{{ $currency_code ?? '$' }}' + parseFloat(val).toFixed(2);
                    
                    document.getElementById('total_overall').textContent = formatCurrency(summary.total_overall);
                ...
                function updateDailyTable(dailyData) {
                    ...
                    dailyData.forEach(item => {
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">${item.count}</td>
