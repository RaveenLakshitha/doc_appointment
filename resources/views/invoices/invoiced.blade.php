...
@section('title', __('file.invoiced_sales'))
...
@section('content')
    <div class="px-4 sm:px-6 lg:px-4 pb-4 sm:py-12 pt-20" x-data="{ 
        ...
        <div class=" flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
                <h1 class="text-2xl sm:text-3xl font-semibold text-gray-900 dark:text-white">
                    {{ __('file.invoiced_sales') }}
                </h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    {{ __('file.manage_invoiced_sales') }}
                </p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div class="overflow-x-auto">
                <style>
                    ...
                    /* Column widths */
                    #docapp-table.dataTable colgroup col:nth-child(1)  { width: 36px  !important; }
                    #docapp-table.dataTable colgroup col:nth-child(2)  { width: 130px !important; }
                    #docapp-table.dataTable colgroup col:nth-child(3)  { width: 100px !important; }
                    #docapp-table.dataTable colgroup col:nth-child(4)  { width: 90px  !important; }
                    #docapp-table.dataTable colgroup col:nth-child(5)  { width: 88px  !important; }
                    #docapp-table.dataTable colgroup col:nth-child(6)  { width: 80px  !important; }
                    #docapp-table.dataTable colgroup col:nth-child(7)  { width: 80px  !important; }
                    #docapp-table.dataTable colgroup col:nth-child(8)  { width: 80px  !important; }
                    #docapp-table.dataTable colgroup col:nth-child(9)  { width: 90px  !important; }
                    #docapp-table.dataTable colgroup col:nth-child(10) { width: 72px  !important; }
                    #docapp-table.dataTable colgroup col:nth-child(11) { width: 80px  !important; }
                    #docapp-table.dataTable colgroup col:nth-child(12) { width: 48px  !important; }
                </style>
                <table id="docapp-table" class="w-full divide-y divide-gray-200 dark:divide-gray-700" style="table-layout:fixed;">
                    <thead class="bg-gray-50 dark:bg-gray-900">
                        <tr>
                            <th class="px-2 py-2 text-left w-12 no-export">
                                <input type="checkbox" id="select-all"
                                    class="w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                            </th>
                             <th class="px-2 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider align-top">
                                 {!! str_replace(' ', '<br>', __('file.invoice_number')) !!}
                             </th>
                             <th class="px-2 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider align-top">
                                 {!! str_replace(' ', '<br>', __('file.patient')) !!}
                             </th>
                             <th class="px-2 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider align-top">
                                 {!! str_replace(' ', '<br>', __('file.therapist')) !!}
                             </th>
                             <th class="px-2 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider align-top">
                                 {!! str_replace(' ', '<br>', __('file.invoice_date')) !!}
                             </th>
                             <th class="px-2 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider align-top">
                                 {!! str_replace(' ', '<br>', __('file.total')) !!}
                             </th>
                             <th class="px-2 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider align-top">
                                 {!! str_replace(' ', '<br>', __('file.balance_due')) !!}
                             </th>
                             <th class="px-2 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider align-top">
                                 {!! str_replace(' ', '<br>', __('file.status')) !!}
                             </th>
                             <th class="px-2 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider align-top">
                                 {!! str_replace(' ', '<br>', __('file.consulate_status')) !!}
                             </th>
                             <th class="px-2 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider align-top">
                                 {!! str_replace(' ', '<br>', __('file.method')) !!}
                             </th>
                             <th class="px-2 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider align-top">
                                 {!! str_replace(' ', '<br>', __('file.email_status')) !!}
                             </th>
                            <th class="px-3 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider no-export align-top">
                                {!! str_replace(' ', '<br>', __('file.actions')) !!}
                            </th>
                        </tr>
                    </thead>
        ...
        <!-- Send Document Modal -->
        <div id="send-document-modal" class="fixed inset-0 z-[100] hidden">
            <div class="fixed inset-0 bg-black/60 backdrop-blur-sm" id="send-modal-backdrop"></div>
            <div class="fixed inset-0 overflow-y-auto">
                <div class="flex min-h-full items-center justify-center p-4">
                    <div class="relative w-full max-w-lg bg-white dark:bg-gray-900 rounded-2xl shadow-2xl overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                                {{ __('file.select_invoice_to_send') }}
                            </h3>
                            <button type="button" id="close-send-modal" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="p-6">
                            <div class="mb-6">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    {{ __('file.upload_invoice_to_send') }}
                                </label>
                                <div class="relative flex items-center justify-center w-full">
                                    <label class="flex flex-col items-center justify-center w-full h-40 border-2 border-gray-300 border-dashed rounded-xl cursor-pointer bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                                        <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                            <svg class="w-10 h-10 mb-4 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                                            </svg>
                                            <p class="mb-2 text-sm text-gray-500 dark:text-gray-400 font-semibold" id="file-name-display">{{ __('file.click_to_upload') }}</p>
                                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('file.max_file_size') }}</p>
                                        </div>
                                        <input type="file" id="manual-file-input" class="hidden" accept=".pdf,.jpg,.jpeg,.png" />
                                    </label>
                                </div>
                            </div>

                            <div class="flex gap-3">
                                <button type="button" id="cancel-send-btn" class="flex-1 px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 transition">
                                    {{ __('file.cancel') }}
                                </button>
                                <button type="button" id="confirm-send-btn" class="flex-1 px-4 py-2 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition disabled:opacity-50">
                                    {{ __('file.send_email') }}
                                </button>
                            </div>
                        </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                const table = $('#docapp-table').DataTable({
                    ...
                    ajax: {
                        ...
                        data: d => {
                            ...
                            d.to = $('#filter-to').val();
                            d.is_printed = 1; // Always show only invoiced sales
                        }
                    },
                    columnDefs: [
                    ...
                    columns: [
                        { data: 'id', width: '36px', render: d => `<input type="checkbox" name="ids[]" value="${d}" class="row-checkbox w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">`, className: 'text-center' },
                        { data: 'invoice_number', width: '130px' },
                        { data: 'patient_name', width: '100px', render: d => `<span title="${d||''}">` + (d||'-') + '</span>' },
                        { data: 'therapist_name', width: '90px', render: d => `<span title="${d||''}">` + (d||'-') + '</span>' },
                        { data: 'invoice_date', width: '88px' },
                        { data: 'total', width: '80px', className: 'text-right font-medium' },
                        { data: 'balance_due', width: '80px', className: 'text-right font-medium' },
                        { data: 'status_html', width: '80px' },
                        { data: 'consulate_status_html', width: '90px' },
                        { data: 'payment_method', width: '72px' },
                        { data: 'is_emailed_html', width: '80px', className: 'text-center' },
                        {
                            data: null,
                            width: '48px',
                            render: (data, type, row) => {
                                ...
                                if (parseFloat(row.balance_due_raw) > 0) {
                                    paymentItem = `<button type="button" onclick="invDd.close();window.dispatchEvent(new CustomEvent('open-payment-drawer',{detail:{id:${row.id},number:'${row.invoice_number}',balance:${row.balance_due_raw}}}))" class="flex w-full items-center gap-2 px-3 py-1.5 text-sm text-green-600 dark:text-green-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></svg>{{ __('file.add_payment') }}</button>`;
                                }
                                return `<div class="flex justify-end"><div class="relative inline-block"><button type="button" onclick="invDd.toggle(this,'idd-${row.id}')" class="p-1.5 rounded-md text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="5" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="12" cy="19" r="2"/></svg></button><div id="idd-${row.id}" class="hidden fixed z-[200] mt-1 w-44 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 py-1"><a href="${row.show_url}${row.show_url.includes('?') ? '&' : '?'}from=invoiced" class="flex items-center gap-2 px-3 py-1.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md"><svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>{{ __('file.view') }}</a><a href="${row.print_url}" class="flex items-center gap-2 px-3 py-1.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md"><svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>{{ __('file.print') }}</a><button type="button" onclick="invDd.close();sendInvoiceEmail(${row.id})" class="flex w-full items-center gap-2 px-3 py-1.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md"><svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>{{ __('file.send_email') }}</button>${paymentItem}<hr class="my-1 border-gray-200 dark:border-gray-600"><button type="button" onclick="invDd.close();confirmDelete('${row.delete_url}')" class="flex w-full items-center gap-2 px-3 py-1.5 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-md"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>{{ __('file.delete') }}</button></div></div></div>`;
                            },
                            ...
                            buttons: [
                                { extend: 'pageLength', className: 'inline-flex items-center gap-2 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 text-sm font-medium transition shadow-sm' },
                                {
                                    extend: 'collection', text: "{{ __('file.Export') }}", className: 'bg-gray-800 hover:bg-gray-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 text-sm font-medium', buttons: [
                                        ...
                                        { extend: 'excel', text: 'Excel', filename: '{{ __("file.bills") }}_{{ date("Y-m-d") }}', exportOptions: { columns: ':not(.no-export)' } },
                                        { extend: 'csv', text: 'CSV', filename: '{{ __("file.bills") }}_{{ date("Y-m-d") }}', exportOptions: { columns: ':not(.no-export)' } },
                                        { extend: 'pdf', text: 'PDF', filename: '{{ __("file.bills") }}_{{ date("Y-m-d") }}', title: '{{ __("file.bill_list") }}', exportOptions: { columns: ':not(.no-export)' } },
                ...
                window.sendInvoiceEmail = function(id) {
                    ...
                    fileInput.value = '';
                    fileNameDisplay.innerText = '{{ __("file.click_to_upload") }}';
                    ...
                    confirmBtn.onclick = function() {
                        ...
                        if (!manualFile) {
                            if (typeof showNotification === 'function') showNotification('{{ __("file.error") }}', '{{ __("file.please_select_invoice_to_upload") }}', 'error');
                            return;
                        }
                        ...
                        $.ajax({
                            ...
                            success: function(res) {
                                ...
                                if (res.success) {
                                    ...
                                    if (typeof showNotification === 'function') showNotification('{{ __("file.error") }}', res.message, 'error');
                                }
                            },
                            error: function(xhr) {
                                confirmBtn.disabled = false;
                                confirmBtn.innerHTML = '{{ __("file.send_email") }}';
                                const msg = xhr.responseJSON?.message || '{{ __("file.failed_to_send_email") }}';
                        ...
                        success: function (response) {
                            if (response.success) {
                                ...
                                if (response.invoice_id) {
                                    const printUrl = '{{ url("invoices") }}/' + response.invoice_id + '/print?redirect=index';
                ...
                window.invDd = {
                    toggle(btn, menuId) {
                        ...
                        document.querySelectorAll('[id^="idd-"]').forEach(m => m.classList.add('hidden'));
                        if (isHidden) {
                    ...
                    close() { document.querySelectorAll('[id^="idd-"]').forEach(m => m.classList.add('hidden')); }
                };
                document.addEventListener('click', function(e) {
                    if (!e.target.closest('[id^="idd-"]') && !e.target.closest('button[onclick*="invDd.toggle"]')) { invDd.close(); }
                });
